...
/* Semaphore used for priority scheduling */
...
void thread_set_priority (int new_priority)
{
  ...
  // Jake started driving
  bool do_change = true;
  struct list_elem *e = list_begin (&t->lock_waiters);
  for (e = list_begin (&t->lock_waiters); e != list_end (&t->lock_waiters);
       e = list_next (e))
    {
      struct lock *cur = list_entry (e, struct lock, lock_elem);
      if (!list_empty (&cur->semaphore.waiters) &&
          cur->prior_priority > new_priority)
        {
          cur->prior_priority = new_priority;
          do_change = false;
        }
